...
public class UnitSelectionBox : MonoBehaviour
{
    ...
    public void SelectAll(GameObject aUnit)
    {

        selectionBox = new Rect();
        startPosition = new Vector2(0, 0);
        endPosition = new Vector2(myCam.orthographicSize * myCam.aspect * sizeX, myCam.orthographicSize * sizeY);
    ...
    void SelectUnits()
    {
        foreach (var unit in UnitSelectionManager.Instance.myAllUnitsList)
        {
    ...
    void SelectUnitsWithtag(GameObject aUnit)
    {
        foreach (var unit in UnitSelectionManager.Instance.myAllUnitsList)
        {
            if (unit.CompareTag(aUnit.tag))
            {
